...
from tensorflow.keras.layers import LSTM, Dense, Dropout, BatchNormalization, Input, TimeDistributed, Flatten
...
MOTION_MODEL_PATH    = 'word_motion_model.h5'
FACE_MODEL_PATH      = 'word_face_model.pkl'
CLASSES_PATH         = 'word_classes.npy'
SEQUENCE_LEN         = 40
HAND_FEATURES        = 132   # 126 shape + 6 wrist-trajectory (see collect_data_words.py)
FACE_FEATURES        = 1404
AUGMENT_COUNT        = 4   # augmented copies per original sample
...
def normalize_hand_sequence(hand_seq):
    """Scale a (T, 132) hand sequence to be scale-invariant.
    Cols 0:63 = left shape, 63:126 = right shape, 126:129 = left wrist
    trajectory, 129:132 = right wrist trajectory. Each hand's shape AND its
    trajectory are divided by that hand's own wrist-to-middle-tip span in
    frame 0, so a sign performed close to or far from the camera looks the
    same to the model.

    CRITICAL: predict.py must call this exact function on live data before
    feeding it to the model — training and inference must see the same
    distribution or predictions will be meaningless."""
...
def _rot_scale(block, cos, sin, scale):
    """Rotate xy in-plane and scale; leaves z untouched. block: (T, N, 3)."""
    xy = block[..., :2]
    z  = block[..., 2:3]
...
def augment_hand_sequence(hand_seq, rng, angle_deg=8, scale_range=(0.9, 1.1)):
    """Rotate + scale a hand sequence (shape AND trajectory columns) to
    create a training variant."""
    seq = np.asarray(hand_seq, dtype=np.float32)
    T = seq.shape[0]
...
def load_dataset():
    if not os.path.exists(DATA_PATH):
        raise FileNotFoundError(f"'{DATA_PATH}' does not exist. Run collect_data_words.py first.")
    ...
    for word in words:
        ...
        for hf in hand_files:
            ...
            # hand_seq expected (SEQUENCE_LEN, HAND_FEATURES)
    ...
    if not hand_seqs:
        raise ValueError("No valid samples found. Re-record with collect_data_words.py.")

    # Normalize hand sequences (scale-invariant) — same for train & predict
...
def build_motion_model(num_classes):
    """LSTM on normalized wrist-relative hand trajectory."""
    model = Sequential([
...
# ── Main ────────────────────────────────────────────────────────────────
def main():
    print("=== FSL DUAL-GATE MODEL TRAINING PIPELINE (Improved) ===\n")
    ...
    print("\nRun predict.py to start the translator.")
